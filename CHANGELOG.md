...
- Timeline: preserve a solver's own events (info, intermediate scores, model calls) when the solver also runs a single agent (e.g. via `agent.run()`); previously they were dropped.
- Timeline: include top-level events recorded outside the init/solvers/scorers spans (e.g. sample limits from an interrupted eval); previously they were dropped.
